...
bool IsInFrustum(
	...
	for (int i = 0; i < 6; i++) {
		...
		if (ans + radius < 0) {		// ＢＳの半径を考慮してフラスタム内かををチェック
...
void DX11Vec2Normalize(XMFLOAT2& invec, XMFLOAT2& outvec) {

	ALIGN16 XMVECTOR inv;
	ALIGN16 XMVECTOR outv;

	inv = XMLoadFloat2(&invec);

	outv = DirectX::XMVector2Normalize(inv);

	XMStoreFloat2(&outvec, outv);
...
/// ３点から法線を求める
/// </summary>
 XMFLOAT3 CreateNormal(XMFLOAT3 p1, XMFLOAT3 p2, XMFLOAT3 p3) {
 ...
 /// 行列から軸を取り出す
 /// </summary>
 void GetAxisXfromMatrix(const XMFLOAT4X4& mtx, XMFLOAT4& out) {
	 out.x = mtx._11;
	 out.y = mtx._12;
	 out.z = mtx._13;
	 out.w = 0.0f;
 }
 void GetAxisYfromMatrix(const XMFLOAT4X4& mtx, XMFLOAT4& out) {
	 out.x = mtx._21;
	 out.y = mtx._22;
	 out.z = mtx._23;
	 out.w = 0.0f;
 }
 void GetAxisZfromMatrix(const XMFLOAT4X4& mtx, XMFLOAT4& out) {
	 out.x = mtx._31;
	 out.y = mtx._32;
	 out.z = mtx._33;
	 out.w = 0.0f;
