...
bool CDirectXGraphics::Init(HWND hWnd, unsigned int Width, unsigned int Height, bool fullscreen)
{
   ...
   // 機能レベル
	...
	// set up the description of the stencl m_state
	...
	// create the depth stencil m_state
	...
	rasterDesc.DepthBiasClamp = 0.1f;
	...
	// create the rasterrizer m_state from the description we just filled out 
	hr = m_lpDevice->CreateRasterizerState(&rasterDesc, &m_rasterState);
	if (FAILED(hr)) {
		return false;
	}

	//  set the rasterizer m_state
	...
	// サンプラーステート設定
	D3D11_SAMPLER_DESC smpDesc0;
	ZeroMemory(&smpDesc0, sizeof(smpDesc0));
	smpDesc0.Filter = D3D11_FILTER_MIN_MAG_MIP_LINEAR;
	smpDesc0.AddressU = D3D11_TEXTURE_ADDRESS_WRAP;
	smpDesc0.AddressV = D3D11_TEXTURE_ADDRESS_WRAP;
	smpDesc0.AddressW = D3D11_TEXTURE_ADDRESS_WRAP;
	smpDesc0.BorderColor[0] = 1.0f;
	smpDesc0.BorderColor[1] = 1.0f;
	smpDesc0.BorderColor[2] = 1.0f;
	smpDesc0.BorderColor[3] = 1.0f;
	smpDesc0.ComparisonFunc = D3D11_COMPARISON_LESS_EQUAL;
	smpDesc0.Filter = D3D11_FILTER_COMPARISON_MIN_MAG_MIP_LINEAR;
	smpDesc0.MaxAnisotropy = 1;
	smpDesc0.MipLODBias = 0;

	D3D11_SAMPLER_DESC smpDesc1;
	ZeroMemory(&smpDesc1, sizeof(smpDesc1));
	smpDesc1.Filter = D3D11_FILTER_MIN_MAG_MIP_LINEAR;
	smpDesc1.AddressU = D3D11_TEXTURE_ADDRESS_WRAP;
	smpDesc1.AddressV = D3D11_TEXTURE_ADDRESS_WRAP;
	smpDesc1.AddressW = D3D11_TEXTURE_ADDRESS_WRAP;
	smpDesc1.BorderColor[0] = 1.0f;
	smpDesc1.BorderColor[1] = 1.0f;
	smpDesc1.BorderColor[2] = 1.0f;
	smpDesc1.BorderColor[3] = 1.0f;
	smpDesc1.ComparisonFunc = D3D11_COMPARISON_GREATER;
	smpDesc1.Filter = D3D11_FILTER_COMPARISON_MIN_MAG_MIP_LINEAR;
	smpDesc1.MaxAnisotropy = 1;
	smpDesc1.MipLODBias = 0;
	...
	//smpDesc.MaxLOD = +FLT_MAX;
		...
		// サンプラーステート生成
	hr = m_lpDevice->CreateSamplerState(&smpDesc0, &m_samplerstate[0]);
	if (FAILED(hr))
	{
		return false;
	}

	hr = m_lpDevice->CreateSamplerState(&smpDesc1, &m_samplerstate[1]);
	if (FAILED(hr))
	{
		return false;
	}
	...
	// サンプラーステートを転送
	m_lpImmediateContext->PSSetSamplers(0, 1, &m_samplerstate[0]);
	m_lpImmediateContext->PSSetSamplers(1, 1, &m_samplerstate[1]);
