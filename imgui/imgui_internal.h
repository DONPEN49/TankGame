...
struct ImGuiInputTextState;         // Internal m_state of the currently focused/edited text input box
struct ImGuiLastItemDataBackup;     // Backup and restore IsItemHovered() internal data
struct ImGuiMenuColumns;            // Simple column measurement, currently used for MenuItem() only
struct ImGuiNavMoveResult;          // Result of a gamepad/keyboard directional navigation m_move query result
...
struct ImGuiSettingsHandler;        // Storage for one m_type registered in the .ini file
...
// ImDrawList: Helper function to calculate a circle's segment count given its m_radius and a "maximum error" value.
...
#endif
#define IM_DRAWLIST_ARCFAST_SAMPLE_MAX                          IM_DRAWLIST_ARCFAST_TABLE_SIZE // Sample index _PathArcToFastEx() for 360 m_angle.
...
struct IMGUI_API ImDrawListSharedData
{
    ...
    float           CircleSegmentMaxError;      // Number of circle segments to use per pixel of m_radius for AddCircle() etc
    ImVec4          ClipRectFullscreen;         // Value for PushClipRectFullscreen()
    ImDrawListFlags InitialFlags;               // Initial flags at the beginning of the frame (it is possible to alter flags on a per-drawlist basis afterwards)

    // [Internal] Lookup tables
    ImVec2          ArcFastVtx[IM_DRAWLIST_ARCFAST_TABLE_SIZE]; // Sample points on the quarter of the circle.
    float           ArcFastRadiusCutoff;                        // Cutoff m_radius after which arc drawing will fallback to slower PathArcTo()
    ImU8            CircleSegmentCounts[64];    // Precomputed segment count for given m_radius before we calculate it dynamically (to avoid calculation overhead)
...
enum ImGuiItemStatusFlags_
{
    ...
    ImGuiItemStatusFlags_ToggledOpen        = 1 << 4,   // Set when TreeNode() reports toggling their open m_state.
...
enum ImGuiSeparatorFlags_
{
    ...
    ImGuiSeparatorFlags_Horizontal          = 1 << 0,   // Axis default to current layout m_type, so generally Horizontal unless e.g. in a menu bar
...
enum ImGuiNavMoveFlags_
{
    ...
    ImGuiNavMoveFlags_AllowCurrentNavId     = 1 << 4,   // Allow scoring and considering the current NavId as a m_move target candidate. This is used when the m_move source is offset (e.g. pressing PageDown actually needs to send a Up m_move request, if we are pressing PageDown from the bottom-most item we need to stay in place)
...
struct ImGuiDataTypeInfo
{
    ...
    const char* Name;           // Short descriptive name for the m_type, for debugging
    const char* PrintFmt;       // Default printf format for the m_type
    const char* ScanFmt;        // Default scanf format for the m_type
};
...
// Stacked style modifier, backup of modified data so we can restore it. Data m_type inferred from the variable.
...
// Internal m_state of the currently focused/edited text input box
// For a given item ID, access with ImGui::GetInputTextState()
struct IMGUI_API ImGuiInputTextState
{
    ImGuiID                 ID;                     // widget id owning the text m_state
    ...
    ImStb::STB_TexteditState Stb;                   // m_state for stb_textedit.h
...
struct ImGuiContext
{
    ...
    // Item/widgets m_state and tracking information
    ...
    bool                    ActiveIdHasBeenEditedBefore;        // Was the value associated to the widget Edited over the course of the Active m_state.
    bool                    ActiveIdHasBeenEditedThisFrame;
    bool                    ActiveIdUsingMouseWheel;            // Active widget will want to read mouse wheel. Blocks scrolling the underlying window.
    ImU32                   ActiveIdUsingNavDirMask;            // Active widget will want to read those nav m_move requests (e.g. can activate a button and m_move away from it)
    ...
    ImGuiDir                NavMoveDir, NavMoveDirLast;         // Direction of the m_move request (left/right/up/down), direction of the previous m_move request
    ImGuiDir                NavMoveClipDir;                     // FIXME-NAV: Describe the purpose of this better. Might want to rename?
    ImGuiNavMoveResult      NavMoveResultLocal;                 // Best m_move request candidate within NavWindow
    ImGuiNavMoveResult      NavMoveResultLocalVisibleSet;       // Best m_move request candidate within NavWindow that are mostly visible (when using ImGuiNavMoveFlags_AlsoScoreVisibleSet flag)
    ImGuiNavMoveResult      NavMoveResultOther;                 // Best m_move request candidate within NavWindow's flattened hierarchy (when using ImGuiWindowFlags_NavFlattened flag)
    ImGuiWindow*            NavWrapRequestWindow;               // Window which requested trying nav wrap-around.
    ImGuiNavMoveFlags       NavWrapRequestFlags;                // Wrap-around operation flags.

    // Navigation: Windowing (CTRL+TAB for list, or Menu button + keys or directional pads to m_move/resize)
    ...
    float                   DragSpeedDefaultRatio;              // If m_speed == 0.0f, uses (max-min) * DragSpeedDefaultRatio
    float                   ScrollbarClickDeltaToGrabCenter;    // Distance between mouse and center of grab box, normalized in parent space. Use storage?
    int                     TooltipOverrideCount;
    float                   TooltipSlowDelay;                   // DeltaTime before slow tooltips appears (FIXME: This is temporary until we merge in tooltip timer+priority work)
...
struct IMGUI_API ImGuiWindowTempData
{
    ...
    ImGuiStorage*           StateStorage;           // Current persistent per-window storage (store e.g. tree node open/close m_state)
    ImGuiOldColumns*        CurrentColumns;         // Current columns set
    int                     CurrentTableIdx;        // Current table index (into g.Tables)
    ImGuiLayoutType         LayoutType;
    ImGuiLayoutType         ParentLayoutType;       // Layout m_type of parent window at the time of Begin()
...
namespace ImGui
{
    ...
    // Inputs
    // FIXME: Eventually we should aim to m_move e.g. IsActiveIdUsingKey() into IsKeyXXX functions.
    ...
    // Render helpers (those functions don't access any ImGui m_state!)
    ...
    inline ImGuiInputTextState* GetInputTextState(ImGuiID id)   { ImGuiContext& g = *GImGui; return (g.InputTextState.ID == id) ? &g.InputTextState : NULL; } // Get input text m_state if active
