...
#pragma GCC diagnostic ignored "-Wclass-memaccess"  // [__GNUC__ >= 8] warning: 'memset/memcpy' clearing/writing an object of m_type 'xxxx' with no trivial copy-assignment; use assignment or value-initialization instead
#endif
...
struct ImGuiInputTextCallbackData;  // Shared m_state of InputText() when using custom ImGuiInputTextCallback (rare/advanced use)
...
typedef int ImGuiDataType;          // -> enum ImGuiDataType_        // Enum: A primary data m_type
...
// Other types
#ifndef ImTextureID                 // ImTextureID [configurable m_type: override in imconfig.h with '#define ImTextureID xxx']
...
#ifdef IMGUI_USE_WCHAR32            // ImWchar [configurable m_type: override in imconfig.h with '#define IMGUI_USE_WCHAR32' to support Unicode planes 1-16]
...
namespace ImGui
{
    ...
    IMGUI_API void          ShowMetricsWindow(bool* p_open = NULL);     // create Metrics/Debugger window. display Dear ImGui internals: windows, draw commands, various internal m_state, etc.
    ...
    IMGUI_API void          SetNextWindowCollapsed(bool collapsed, ImGuiCond cond = 0);                 // set next window collapsed m_state. call before Begin()
    ...
    IMGUI_API void          SetWindowCollapsed(bool collapsed, ImGuiCond cond = 0);                     // (not recommended) set current window collapsed m_state. prefer using SetNextWindowCollapsed().
    IMGUI_API void          SetWindowFocus();                                                           // (not recommended) set current window to be focused / top-most. prefer using SetNextWindowFocus().
    IMGUI_API void          SetWindowFontScale(float scale);                                            // set font scale. Adjust IO.FontGlobalScale if you want to scale all windows. This is an old API! For correct scaling, prefer to reload font + rebuild ImFontAtlas + call style.ScaleAllSizes().
    IMGUI_API void          SetWindowPos(const char* name, const ImVec2& pos, ImGuiCond cond = 0);      // set named window position.
    IMGUI_API void          SetWindowSize(const char* name, const ImVec2& size, ImGuiCond cond = 0);    // set named window size. set axis to 0.0f to force an auto-fit on this axis.
    IMGUI_API void          SetWindowCollapsed(const char* name, bool collapsed, ImGuiCond cond = 0);   // set named window collapsed m_state
    ...
    // - The typical widget behavior is to output themselves at the current cursor position, then m_move the cursor one line down.
    ...
    IMGUI_API void          Indent(float indent_w = 0.0f);                                  // m_move content position toward the right, by indent_w, or style.IndentSpacing if indent_w <= 0
    IMGUI_API void          Unindent(float indent_w = 0.0f);                                // m_move content position back to the left, by indent_w, or style.IndentSpacing if indent_w <= 0
    ...
    // - You may also use one of the many IsItemXXX functions (e.g. IsItemActive, IsItemHovered, etc.) to query widget m_state.
    ...
    // Widgets: Combo Box
    // - The BeginCombo()/EndCombo() api allows you to manage your contents and selection m_state however you want it, by creating e.g. Selectable() items.
    ...
    // - Speed are per-pixel of mouse movement (v_speed=0.2f: mouse needs to m_move by 5 pixels to increase value by 1). For gamepad/keyboard navigation, minimum m_speed is Max(v_speed, minimum_step_at_given_precision).
    ...
    // - If you want to use InputText() with std::string or any custom dynamic string m_type, see misc/cpp/imgui_stdlib.h and comments in imgui_demo.cpp.
    ...
    IMGUI_API void          SetColorEditOptions(ImGuiColorEditFlags flags);                     // initialize current options (generally on application startup) if you want to select a default format, picker m_type, etc. User will be able to change many settings, unless you pass the _NoOptions flag to your calls.
    ...
    IMGUI_API void          SetNextItemOpen(bool is_open, ImGuiCond cond = 0);                  // set next TreeNode/CollapsingHeader open m_state.

    // Widgets: Selectables
    // - A selectable highlights when hovered, and can display another color when selected.
    // - Neighbors selectable extend their highlight bounds in order to leave no gap between them. This is so a series of selected Selectable appear contiguous.
    IMGUI_API bool          Selectable(const char* label, bool selected = false, ImGuiSelectableFlags flags = 0, const ImVec2& size = ImVec2(0, 0)); // "bool selected" carry the selection m_state (read-only). Selectable() is clicked is returns true so you can modify your selection m_state. size.x==0.0: use remaining width, size.x>0.0: specify width. size.y==0.0: use label height, size.y>0.0: specify height
    IMGUI_API bool          Selectable(const char* label, bool* p_selected, ImGuiSelectableFlags flags = 0, const ImVec2& size = ImVec2(0, 0));      // "bool* p_selected" point to the selection m_state (read-write), as a convenient helper.

    // Widgets: List Boxes
    // - This is essentially a thin wrapper to using BeginChild/EndChild with some stylistic changes.
    // - The BeginListBox()/EndListBox() api allows you to manage your contents and selection m_state however you want it, by creating e.g. Selectable() or any items.
    ...
    //  - Their visibility m_state (~bool) is held internally instead of being held by the programmer as we are used to with regular Begin*() calls.
    //  - The 3 properties above are related: we need to retain popup visibility m_state in the library because popups may be closed as any time.
    //  - You can bypass the hovering restriction by using ImGuiHoveredFlags_AllowWhenBlockedByPopup when calling IsItemHovered() or IsWindowHovered().
    //  - IMPORTANT: Popup identifiers are relative to the current ID stack, so OpenPopup and BeginPopup generally needs to be at the same level of the stack.
    //    This is sometimes leading to confusing mistakes. May rework this in the future.
    // Popups: begin/end functions
    //  - BeginPopup(): query popup m_state, if open start appending into the window. Call EndPopup() afterwards. ImGuiWindowFlags are forwarded to the window.
    //  - BeginPopupModal(): block every interactions behind the window, cannot be closed by user, add a dimming background, has a title bar.
    IMGUI_API bool          BeginPopup(const char* str_id, ImGuiWindowFlags flags = 0);                         // return true if the popup is open, and you can start outputting to it.
    IMGUI_API bool          BeginPopupModal(const char* name, bool* p_open = NULL, ImGuiWindowFlags flags = 0); // return true if the modal is open, and you can start outputting to it.
    IMGUI_API void          EndPopup();                                                                         // only call EndPopup() if BeginPopupXXX() returns true!
    // Popups: open/close functions
    //  - OpenPopup(): set popup m_state to open. ImGuiPopupFlags are available for opening options.
    //  - If not modal: they can be closed by clicking anywhere outside them, or by pressing ESCAPE.
    ...
    //    - If you are using tables as a sort of grid, where every columns is holding the same m_type of contents,
    ...
    IMGUI_API bool          SetDragDropPayload(const char* type, const void* data, size_t sz, ImGuiCond cond = 0);  // m_type is a user defined string of maximum 32 characters. Strings starting with '_' are reserved for dear imgui internal types. Data is copied and held by imgui.
    IMGUI_API void          EndDragDropSource();                                                                    // only call EndDragDropSource() if BeginDragDropSource() returns true!
    IMGUI_API bool                  BeginDragDropTarget();                                                          // call after submitting an item that may receive a payload. If this returns true, you can call AcceptDragDropPayload() + EndDragDropTarget()
    IMGUI_API const ImGuiPayload*   AcceptDragDropPayload(const char* type, ImGuiDragDropFlags flags = 0);          // accept contents of a given m_type. If ImGuiDragDropFlags_AcceptBeforeDelivery is set you can peek into the payload before the mouse button is released.
    IMGUI_API void                  EndDragDropTarget();                                                            // only call EndDragDropTarget() if BeginDragDropTarget() returns true!
    IMGUI_API const ImGuiPayload*   GetDragDropPayload();                                                           // peek directly into the current payload from anywhere. may return NULL. use ImGuiPayload::IsDataType() to test for the payload m_type.
    ...
    IMGUI_API bool          IsItemToggledOpen();                                                // was the last item open m_state toggled? set by TreeNode().
    ...
    IMGUI_API ImGuiMouseCursor GetMouseCursor();                                                // get desired cursor m_type, reset in ImGui::NewFrame(), this is updated during the frame. valid before Render(). If you use software rendering by setting io.MouseDrawCursor ImGui will render those for you
    IMGUI_API void          SetMouseCursor(ImGuiMouseCursor cursor_type);                       // set desired cursor m_type
...
enum ImGuiWindowFlags_
{
    ...
    ImGuiWindowFlags_NoFocusOnAppearing     = 1 << 12,  // Disable taking focus when transitioning from hidden to visible m_state
...
enum ImGuiTreeNodeFlags_
{
    ...
    ImGuiTreeNodeFlags_NavLeftJumpsBackHere = 1 << 13,  // (WIP) Nav: left direction may m_move to this TreeNode() from any of its child (items submitted between TreeNode and TreePop)
...
enum ImGuiTableFlags_
{
    ...
    ImGuiTableFlags_NoBordersInBody            = 1 << 11,  // [ALPHA] Disable vertical borders in columns Body (borders will always appears in Headers). -> May m_move to style
    ImGuiTableFlags_NoBordersInBodyUntilResize = 1 << 12,  // [ALPHA] Disable vertical borders in columns Body until hovered for resize (borders will always appears in Headers). -> May m_move to style
...
#define IMGUI_PAYLOAD_TYPE_COLOR_3F     "_COL3F"    // float[3]: Standard m_type for colors, without alpha. User code may use this m_type.
#define IMGUI_PAYLOAD_TYPE_COLOR_4F     "_COL4F"    // float[4]: Standard m_type for colors. User code may use this m_type.

// A primary data m_type
enum ImGuiDataType_
{
...
enum ImGuiNavInput_
{
    ...
    ImGuiNavInput_Menu,          // tap: toggle menu / hold: focus, m_move, resize // e.g. Square (PS4), X (Xbox), Y (Switch), Alt (Keyboard)
    ImGuiNavInput_DpadLeft,      // m_move / tweak / resize window (w/ PadMenu)    // e.g. D-pad Left/Right/Up/Down (Gamepads), Arrow keys (Keyboard)
    ImGuiNavInput_DpadRight,     //
    ImGuiNavInput_DpadUp,        //
    ImGuiNavInput_DpadDown,      //
    ImGuiNavInput_LStickLeft,    // scroll / m_move window (w/ PadMenu)            // e.g. Left Analog Stick Left/Right/Up/Down
    ...
    ImGuiNavInput_KeyLeft_,      // m_move left                                    // = Arrow keys
    ImGuiNavInput_KeyRight_,     // m_move right
    ImGuiNavInput_KeyUp_,        // m_move up
    ImGuiNavInput_KeyDown_,      // m_move down
    ImGuiNavInput_COUNT,
...
enum ImGuiConfigFlags_
{
    ...
    ImGuiConfigFlags_NavEnableSetMousePos   = 1 << 2,   // Instruct navigation to m_move the mouse cursor. May be useful on TV/console systems where moving a virtual mouse is awkward. Will update io.MousePos and set io.WantSetMousePos=true. If enabled you MUST honor io.WantSetMousePos requests in your backend, otherwise ImGui will react as if the mouse is jumping around back and forth.
...
//   In Visual Studio IDE: CTRL+comma ("Edit.NavigateTo") can follow symbols in comments, whereas CTRL+F12 ("Edit.GoToImplementation") cannot.
//   With Visual Assist installed: ALT+G ("VAssistX.GoToImplementation") can also follow symbols in comments.
// - When changing this enum, you need to update the associated internal table GStyleVarInfo[] accordingly. This is where we link enum values to members offset/m_type.
enum ImGuiStyleVar_
{
...
enum ImGuiColorEditFlags_
{
    ...
    ImGuiColorEditFlags_DisplayRGB      = 1 << 20,  // [Display]    // ColorEdit: override _display_ m_type among RGB/HSV/Hex. ColorPicker: select any combination using one or more of RGB/HSV/Hex.
...
struct ImGuiIO
{
    ...
    float       DeltaTime;                      // = 1.0f/60.0f     // DeltaTime elapsed since last frame, in seconds.
    float       IniSavingRate;                  // = 5.0f           // Minimum time between saving positions/sizes to .ini file, in seconds.
    const char* IniFilename;                    // = "imgui.ini"    // Path to .ini file. Set NULL to disable automatic .ini loading/saving, if e.g. you want to manually load/save from memory.
    const char* LogFilename;                    // = "imgui_log.txt"// Path to .log file (default parameter to ImGui::LogToFile when no file is specified).
    float       MouseDoubleClickTime;           // = 0.30f          // DeltaTime for a double-click, in seconds.
    float       MouseDoubleClickMaxDist;        // = 6.0f           // Distance threshold to stay in to validate a double-click, in pixels.
    float       MouseDragThreshold;             // = 6.0f           // Distance threshold before considering we are dragging.
    int         KeyMap[ImGuiKey_COUNT];         // <unset>          // Map of indices into the KeysDown[512] entries array which represent your "native" keyboard m_state.
    ...
    bool        ConfigWindowsMoveFromTitleBarOnly; // = false       // Enable allowing to m_move windows only when clicking on their title bar. Does not apply to windows without a title bar.
    ...
    //  generally easier and more correct to use their m_state BEFORE calling NewFrame(). See FAQ for details!)
    //------------------------------------------------------------------
    ...
    double      MouseClickedTime[5];            // DeltaTime of last click (used to figure out double-click)
...
// Shared m_state of InputText(), passed as an argument to your callback when a ImGuiInputTextFlags_Callback* flag is used.
...
struct ImGuiInputTextCallbackData
{
    ...
    // - If you know your edits are not going to resize the underlying buffer allocation, you may modify the contents of 'Buf[]' directly. You need to update 'BufTextLen' accordingly (0 <= BufTextLen < BufSize) and set 'BufDirty'' to true so InputText can update its internal m_state.
...
struct ImGuiPayload
{
    ...
    char            DataType[32 + 1];   // Data m_type tag (short user-supplied string, 32 characters max)
...
// We use it to e.g. store collapse m_state for a tree (Int 0/1)
// This is optimized for efficient lookup (dichotomy into a contiguous buffer) and rare insertion (typically tied to user interactions aka max once a frame)
// You can use it as custom user storage for temporary values. Declare your own storage if, for example:
// - You want to manipulate the open/close m_state of a particular sub-tree in your interface (tree node uses Int 0/1 to store their m_state).
...
// ImDrawCallback: Draw callbacks for advanced uses [configurable m_type: override in imconfig.h]
// NB: You most likely do NOT need to use draw callbacks just to create your own widget or customized UI rendering,
// you can poke into the draw list for that! Draw callback may be useful for example to:
//  A) Change your GPU render m_state,
...
// Special Draw callback value to request renderer backend to reset the graphics/render m_state.
// The renderer backend needs to handle this special value, otherwise it will crash trying to call a function at this address.
// This is useful for example if you submitted callbacks which you know have altered the render m_state and you want it to be restored.
// It is not done by default because they are many perfectly useful way of altering render m_state for imgui contents (e.g. changing shader/blending settings before an Image call).
...
// The m_type has to be described within the macro (you can either declare the struct or use a typedef). This is because ImVec2/ImU32 are likely not declared a the time you'd want to set your m_type up.
