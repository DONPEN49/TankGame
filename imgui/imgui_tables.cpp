...
void ImGui::TableUpdateLayout(ImGuiTable* table)
{
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
    {
        ...
        // Update Enabled m_state, mark settings/sortspecs dirty
...
void ImGui::TableSetupColumn(const char* label, ImGuiTableColumnFlags flags, float init_width_or_weight, ImGuiID user_id)
{
    ...
    if (table->IsInitializing)
    {
        ...
        // Init default visibility/sort m_state
...
// We allow querying for an extra column in order to poll the IsHovered m_state of the right-most section
...
void ImGui::TableSetColumnWidth(int column_n, float width)
{
    ...
    // - W1 F2 F3  resize from W1|          --> ok: equivalent to resizing |F2. F3 will not m_move.
...
// When the contents of a column didn't stray off its limit, we m_move its channels into the corresponding group
...
void ImGui::TableMergeDrawChannels(ImGuiTable* table)
{
    ...
    {
        // We skip channel 0 (Bg0/Bg1) and 1 (Bg2 frozen) from the shuffling since they won't m_move - see channels allocation in TableSetupDrawChannels().
...
// Restore initial m_state of table (with or without saved settings)
...
void ImGui::TableSaveSettings(ImGuiTable* table)
{
    ...
    for (int n = 0; n < table->ColumnsCount; n++, column++, column_settings++)
    {
        ...
        // We skip saving some data in the .ini file when they are unnecessary to restore our m_state.
...
void ImGui::NextColumn()
{
    ...
    // FIXME-COLUMNS: Share code with BeginColumns() - m_move code on columns setup.
    float offset_0 = GetColumnOffset(columns->Current);
    float offset_1 = GetColumnOffset(columns->Current + 1);
    float width = offset_1 - offset_0;
    PushItemWidth(width * 0.65f);
