...
// Widgets
static const float          DRAGDROP_HOLD_TO_OPEN_TIMER = 0.70f;    // DeltaTime for drag-hold to activate items accepting the ImGuiButtonFlags_PressedOnDragDropHold button behavior.
...
// See the series of events below and the corresponding m_state reported by dear imgui:
//------------------------------------------------------------------------------------------------------------------------------------------------
...
bool ImGui::CloseButton(ImGuiID id, const ImVec2& pos)
{
    ...
    // This may better be applied as a general hit-rect reduction mechanism for all widgets to ensure the area to m_move window is always accessible?
...
// NB: v_speed is float to allow adjusting the drag m_speed with more precision
...
// For 32-bit and larger types, slider bounds are limited to half the natural m_type range.
...
#define STB_TEXTEDIT_K_LEFT         0x200000 // keyboard input to m_move cursor left
#define STB_TEXTEDIT_K_RIGHT        0x200001 // keyboard input to m_move cursor right
#define STB_TEXTEDIT_K_UP           0x200002 // keyboard input to m_move cursor up
#define STB_TEXTEDIT_K_DOWN         0x200003 // keyboard input to m_move cursor down
#define STB_TEXTEDIT_K_LINESTART    0x200004 // keyboard input to m_move cursor to start of line
#define STB_TEXTEDIT_K_LINEEND      0x200005 // keyboard input to m_move cursor to end of line
#define STB_TEXTEDIT_K_TEXTSTART    0x200006 // keyboard input to m_move cursor to start of text
#define STB_TEXTEDIT_K_TEXTEND      0x200007 // keyboard input to m_move cursor to end of text
#define STB_TEXTEDIT_K_DELETE       0x200008 // keyboard input to delete selection or character under cursor
#define STB_TEXTEDIT_K_BACKSPACE    0x200009 // keyboard input to delete selection or character left of cursor
#define STB_TEXTEDIT_K_UNDO         0x20000A // keyboard input to perform undo
#define STB_TEXTEDIT_K_REDO         0x20000B // keyboard input to perform redo
#define STB_TEXTEDIT_K_WORDLEFT     0x20000C // keyboard input to m_move cursor left one word
#define STB_TEXTEDIT_K_WORDRIGHT    0x20000D // keyboard input to m_move cursor right one word
#define STB_TEXTEDIT_K_PGUP         0x20000E // keyboard input to m_move cursor up a page
#define STB_TEXTEDIT_K_PGDOWN       0x20000F // keyboard input to m_move cursor down a page
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    // We are only allowed to access the m_state if we are already the active widget.
    ...
    {
        // Access m_state even if we don't own it yet.
    ...
    if (g.ActiveId == id)
    {
        ...
        if (apply_edit_back_to_user_buffer)
        {
            ...
            // FIXME-OPT: CPU waste to do this every time the widget is active, should mark dirty m_state from the stb_textedit callbacks.
            if (!is_readonly)
            {
...
void ImGui::ColorPickerOptionsPopup(const float* ref_col, ImGuiColorEditFlags flags)
{
    ...
    if (allow_opt_picker)
    {
        ...
        {
            // Draw small/thumbnail version of each picker m_type (over an invisible button for selection)
...
bool ImGui::TreeNodeBehaviorIsOpen(ImGuiID id, ImGuiTreeNodeFlags flags)
{
    ...
    if (g.NextItemData.Flags & ImGuiNextItemDataFlags_HasOpen)
    {
        ...
        {
            // We treat ImGuiCond_Once and ImGuiCond_FirstUseEver the same because tree node m_state are not saved persistently.
...
void ImGui::TreePop()
{
    ...
    // Handle Left arrow to m_move to parent tree node (when ImGuiTreeNodeFlags_NavLeftJumpsBackHere is enabled)
...
// Set next TreeNode/CollapsingHeader open m_state.
...
// Do not mistake this with the Open m_state of the header itself, which you can adjust with SetNextItemOpen() or ImGuiTreeNodeFlags_DefaultOpen.
...
void ImGui::EndMenuBar()
{
    ...
    // Nav: When a m_move request within one of our child menu failed, capture the request to navigate among our siblings.
...
void ImGui::EndMenu()
{
    // Nav: When a left m_move request _within our child menu_ failed, close ourselves (the _parent_ menu).
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar)
{
    ...
    {
        // Scrolling m_speed adjust itself so we can always reach our target in 1/3 seconds.
...
void ImGui::TabItemLabelAndCloseButton(ImDrawList* draw_list, const ImRect& bb, ImGuiTabItemFlags flags, ImVec2 frame_padding, const char* label, ImGuiID tab_id, ImGuiID close_button_id, bool is_contents_visible, bool* out_just_closed, bool* out_text_clipped)
{
    ...
    // In Style V2 we'll have full override of all colors per m_state (e.g. focused, selected)
    ...
    // Return clipped m_state ignoring the close button
    if (out_text_clipped)
    {
